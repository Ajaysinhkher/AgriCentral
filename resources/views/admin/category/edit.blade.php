...
<div class="container mx-auto p-4">
    <div class="bg-white rounded-lg shadow-md max-w-2xl mx-auto p-5">
        ...
        <form id="editCategoryForm" action="{{ route('admin.category.update', $category->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <select name="status" class="w-full px-3 py-1.5 border rounded focus:ring focus:ring-blue-200" required>
                    ...
                    <option value="available" {{ old('status', $category->status) == 'available' ? 'selected' : '' }}>Available</option>
                    <option value="unavailable" {{ old('status', $category->status) == 'unavailable' ? 'selected' : '' }}>Unavailable</option>
                </select>
                <span class="error-message text-red-500 text-xs mt-1 block"></span>
            </div>
        ...
        $("#editCategoryForm").validate({
            ...
            messages: {
                ...
                status: {
                    ...
                    inList: "Status must be either available or unavailable."
                }
            },
            errorElement: "span",
